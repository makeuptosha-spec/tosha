...
import { fmt, fmtNum, parseNum, esEsteMes, Icon, ProgressBar, HOGAR_ID } from "../utils.jsx";

const colorPct = (pct) => pct >= 100 ? "var(--danger)" : pct >= 80 ? "var(--warn)" : "var(--success)";

export default function Presupuestos({ presupuestos, setPresupuestos, movimientos, categorias }) {
  ...
  // Solo categorías de gasto del propio usuario (viven en Firestore desde el
  // tab Conf), y sin las que ya tienen presupuesto: uno por categoría.
  const categoriasDisponibles = useMemo(() =>
    categorias
      .filter(c => c.tipo === "gasto")
      .map(c => c.nombre)
      .filter(nombre => !presupuestos.some(p => p.categoria === nombre) || nombre === form.categoria),
    [categorias, presupuestos, form.categoria]
  );
