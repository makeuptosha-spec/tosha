...
export default function LoginScreen() {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", overflow: "hidden", background: "radial-gradient(circle at 20% 30%, #f4fbf8 0%, transparent 50%), radial-gradient(circle at 80% 80%, #d6f3e6 0%, transparent 50%), radial-gradient(circle at 80% 20%, #e3f5ee 0%, transparent 40%), #dcece4" }}>
      ...
      <div style={{ position: "absolute", bottom: "-10%", right: "-5%", width: 250, height: 250, background: "linear-gradient(135deg, #10B981 0%, #6EE7B7 99%, #6EE7B7 100%)", borderRadius: "50%", filter: "blur(2px)", boxShadow: "inset -20px -20px 40px rgba(0,0,0,0.1)" }} />
      ...
      <div className="animate" style={{ background: "rgba(255, 255, 255, 0.45)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", border: "1px solid rgba(255, 255, 255, 0.6)", borderRadius: 32, padding: "40px 30px", width: "100%", maxWidth: 360, boxShadow: "0 8px 32px rgba(0, 0, 0, 0.05)", zIndex: 2, position: "relative" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 36, fontSize: 13, color: "#666" }}><span>Mis Finanzas</span></div>
        <h1 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 28, fontWeight: 500, color: "#111", marginBottom: 28 }}>Iniciar sesión</h1>
        <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginTop: 12 }}>
            ...
            <button type="submit" disabled={cargando} style={{ background: "var(--dark)", color: "#fff", border: "none", borderRadius: 50, width: 56, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", opacity: cargando ? 0.7 : 1 }}>
      ...
      <div className="animate" style={{ background: "var(--dark)", color: "#fff", borderRadius: 32, padding: "36px 30px 24px", width: "90%", maxWidth: 320, marginTop: -30, zIndex: 1, position: "relative", boxShadow: "0 20px 40px rgba(0,0,0,0.15)" }}>
        <h3 style={{ fontSize: 22, fontWeight: 500, marginBottom: 4, letterSpacing: -0.5 }}>Control total</h3>
        <p style={{ color: "#aaa", fontSize: 13 }}>De tus finanzas personales</p>
      </div>
    </div>
  );
}