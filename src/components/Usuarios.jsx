...
export default function Usuarios({ onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", backdropFilter: "blur(6px)", zIndex: 9500, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div className="animate" style={{ background: "#fff", borderRadius: 24, padding: 28, width: "100%", maxWidth: 420, maxHeight: "85vh", overflowY: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }}>
        ...
        {error && <div style={{ background: "#FFEBEE", border: "1px solid #FFCDD2", color: "var(--danger)", fontSize: 12, padding: "8px 12px", borderRadius: 8, marginBottom: 16 }}>{error}</div>}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {usuarios.map(u => (
              <div key={u.email} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--bg)", borderRadius: 12, padding: "10px 14px" }}>
                ...
                <button onClick={() => quitarUsuario(u.email)} style={{ background: "#FFEBEE", color: "var(--danger)", border: "none", borderRadius: 8, width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center" }}>
